...
var zoombox_path = '/global/javascript/zoombox/';
...
$.fn.zoombox = function(opts){
    ...
    return this.each(function(){
        ...
        $(this).unbind('click').click(function(){
            ...
            elem = $(obj);
            link = elem.attr('href');
            if (elem.find('img').data('type') == 'canvas')
                canvas = true;
		...
		else
		{
			if (typeof src[link] === 'undefined')
			{
				loadImage(
					...
					{
						contain: true,
						// pour mettre l'image dans le bon sens
						orientation: true
					}
				);
			}
			...
			{
				width = src[link].width;
				height = src[link].height;
				$('#zoombox_loader').remove();
				setContent();
				open();
			}
...
function gallery(){
    ...
    if(options.gallery){
        ...
        for(var i in imageset){
            ...
            {
				//Modifié par djamel sarni, les miniatures reprenait l'image de grande taille
                imgSrc = imageset[i].find('img').attr('src');
    ...
    $('#zoombox').bind('change',function(e,css){
        ...
        if(css != null){
            d = options.duration;
            center = ( parseInt(css.width) / 2 );
        }else{
            center = parseInt($('#zoombox .zoombox_gallery').width()) / 2;
        }
        var decal = - $('#zoombox .zoombox_gallery img.current').data('left') + $('#zoombox .zoombox_gallery img.current').width() / 2;
        var left =    center + decal  ;
...
function setContent(){
    ...
    if(filtreImg.test(url))
    {
        ...
        if (typeof src[link] !== 'undefined')
        {
			content = loadImage.scale(
				src[link], // img or canvas element
				{
				    maxWidth: width,
					maxHeight: height,
				    minWidth: width,
...
function gotoSlide(i){
    ...
    link = elem.attr('href');
    // if (imageset[position].find('canvas').size())
    // 	src[link] = imageset[position].find('canvas').data('src');
    // else
		// src[link] = link;
